...
void KapibaraUI::drawBottomWorkspace(const Rect &full)
{
        ...
        if(!expanded)
        {
            // Collapsed: a right-arrow at the source edge expands; the Modulators
            // panel (curve/amp-env editors — the grid lives in the toolbar MATRIX
            // view) fills the rest.
            ...
            const Rect modRegion { mx, full.y + 8.0f, full.x + full.w - 8.0f - mx, full.h - 16.0f };
            drawMatrixDashboard(modRegion);
